...
(function (global) {
  ...
  var meetingsCol = makeCollection(KEYS.meetings, 'mtg');
  var usersCol = makeCollection(KEYS.users, 'usr');

  var meetings = Object.assign({}, meetingsCol, {
    seedIfEmpty: function (defaults) {
      if (read(KEYS.meetings, null) === null) write(KEYS.meetings, defaults || []);
      return meetingsCol.getAll();
    },
  ...
  function autoSaveBackup() {
    var previous = read(KEYS.autoBackup, null);
    if (previous && previous.savedAt && Date.now() - new Date(previous.savedAt).getTime() < 86400000) return false;
    var snapshot = exportBackup();
    snapshot.savedAt = nowIso();
    return write(KEYS.autoBackup, snapshot);
  }
  ...
  function restoreBackup(obj) {
    ...
    if (Array.isArray(obj.data.meetings)) write(KEYS.meetings, obj.data.meetings);
  ...
  var ForgeXDB = {
    ...
    backup: { export: exportBackup, download: downloadBackup, autoSave: autoSaveBackup, restore: restoreBackup, validate: validateBackupShape, wipeAll: wipeAllData },
